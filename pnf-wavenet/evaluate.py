...
def snr(gt, output):
    return 10 * np.log10((np.linalg.norm(gt) ** 2) / (np.linalg.norm(gt - output) ** 2))
...
def get_data_loader(data_dir, collate_fn):
    ...
    if X is None:
        ...
        data_loader = data_utils.DataLoader(
            ...
            num_workers=0, sampler=None, shuffle=True,
...
if __name__ == "__main__":
    ...
    writing_dir = join("experiments", "autoregressive_0pt244", random_prefix)
    dst_dir = writing_dir    
    ...
    for idx, (x, y, c, g, input_lengths) in enumerate(test_data_loader):
        ...
        # Generate
        y_hats = batch_wavegen(model, c=c, g=g, fast=True, tqdm=_tqdm, writing_dir=writing_dir)
        # Save each utt.

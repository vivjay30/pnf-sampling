...
class WaveNet(nn.Module):
    ...
    def forward(self, x, c=None, g=None, softmax=False):
        ...
        Args:
            x (Tensor): Scalar or One-hot encoded audio signal, shape (B x C x T)
    ...
    def smoothed_loss(self, input_batch, sigma=1.0, c=None, batched=False):
        ...
        # Forward pass
        if batched: prediction = self.forward(network_input, c=c) # B x 256 x T
        else: prediction = self.forward(network_input.unsqueeze(0), c=c) # 1 x 256 x T
    ...
    def binary_softmax_loss(self, input_batch, c=None):
        """
        input_batch: B x 1 x T
        """
        # Forward pass
        output = self.forward(input_batch, c=c, softmax=False).double()

        # Log softmax
        softmax = nn.Softmax(dim=1)
        softmax_output = softmax(output[:, :, :-1])
        log_softmax = torch.log2(softmax_output)

        # Negative Likelihood
        loss = nn.NLLLoss(reduction="mean")
        target = torch.clamp(input_batch[:, 0, 1:], 0, 255).round().long()
        return loss(log_softmax, target)

...
def write_images(x, fname, itm, n=2):
    ...
    cv2.imwrite(os.path.join('images','inpaint_{}{}_{}.png'.format(fname,args.seed,itm)), panel)

def generate_masked(loader, mask):
    gt,y = next(loader)

    x = mask*torch.randn(*gt.shape) + (1.-mask)*gt.clone()
    return (-1*mask + (1.-mask)*gt).cuda(), x.cuda(), gt.cuda()
...
for itm in range(args.count):
    mask = torch.zeros(obs)
    mask[:,8:24,8:24] = 1 # inpaint

    #mask = torch.ones(obs)
    #mask[:,6:26,6:26] = 0 # outpaint

    masked, x, gt = generate_masked(iter(test_loader), mask)
    write_images(masked.cpu(), 'masked', itm)
...
np.save('opt/inpaint_unsmoothed_nll_lr{}_T{}.npy'.format(step_lr, n_steps_each), nll_history)
np.save('opt/inpaint_nll_lr{}_T{}.npy'.format(step_lr, n_steps_each), smoothed_history)
